...
public class CommonCustomerRepository implements CustomerRepository {
  ...
  public List<Customer> getAll() {
    try (Connection connection = getConnection()) {
      ...
      String query = "select cu.id, cu.name, ca.id, cp.product_id, p.name, p.price, p.is_active from customer as cu "
      ...
      while (resultSet.next()) {
        ...
        // получаем айдишник покупателя
        int customerId = resultSet.getInt(CUSTOMER_ID);
        Customer customer; // объявили, инициализируем в цикле

        // создаём объект покупателя, либо получаем его из мапы,
        // если он уже был ранее создан

        // если создавали
        if (customers.containsKey(customerId)) {
          customer = customers.get(customerId);
          // если не создавали такого покупателя
        } else {
